...
- **Pay only for what you use.** Transcription and clip planning bill your OpenRouter account at their prices, or you can run transcription on your own GPU for free. Rendering happens locally with FFmpeg. BridgeClip shows estimated API cost when the providers return usable usage data.
...
Transcription can also run on your own GPU instead of OpenRouter, with no API call and no cost. **Settings → Transcription → Runs on** switches to `This computer` and lets you pin a device; **Detect** reports the installed backends and devices. That needs NVIDIA's [NeMo-Speech.cpp](https://github.com/NVIDIA/NeMo-Speech.cpp) runtime and a local Nemotron GGUF model, which BridgeClip never downloads for you. See [on-device transcription](docs/transcription.md#on-device-transcription-with-a-gpu).
...
3. Optional: run **Settings → Transcription → Runs on** to switch transcription to your own GPU, and press **Detect** to confirm the runtime and model were found.
