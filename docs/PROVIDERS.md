...
| --- | --- | --- | --- |
| Local ASR | The engine defaults to `nvidia/nemotron-3.5-asr-streaming-0.6b` through the project-local NeMo-Speech runtime. The current bundle is CPU-only. | No provider account or API charge. The model already exists under `engine-bin\models\`. | The path and model are hardcoded defaults with environment overrides. There is no UI inventory or explicit device/model selector. |
