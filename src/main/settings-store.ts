...
export interface AppSettings {
  ...
  /** Where speech is transcribed. 'openrouter' is the default and costs nothing extra. */
...
function normalizeSettings(settings: Partial<AppSettings>): AppSettings {
  ...
  const normalized: AppSettings = {
    ...
    // Only the two supported backends; anything else means the hosted path,
    // which is the behaviour every install had before local transcription.
    transcriptionBackend: settings.transcriptionBackend === 'nemotron' ? 'nemotron' : 'openrouter',
    // A device outside the documented set falls back to automatic selection
    // rather than being forwarded to the runtime.
...
export function loadSettings(): AppSettings {
  ...
  try {
    ...
    const settings = normalizeSettings({
      ...
      transcriptionBackend: typeof raw.transcriptionBackend === 'string'
        ? raw.transcriptionBackend as 'openrouter' | 'nemotron'
        : DEFAULT_SETTINGS.transcriptionBackend,
      transcriptionDevice: typeof raw.transcriptionDevice === 'string'
        ? raw.transcriptionDevice
        : DEFAULT_SETTINGS.transcriptionDevice
    })
...
export function getSettingsForBridge(settings: AppSettings): Record<string, string> {
  return {
    ...
    // Validated on write, so these are always documented values.
