...
export function TranscriptionSettings({
  ...
  return <Panel>
    <PanelHeader
      ...
      title="Transcription"
      description="Turn speech into text with timestamps. On-device transcription keeps the audio on this computer."
      action={
        <Button
          size="sm"
          onClick={onRefresh}
    ...
    <div className="mt-4 grid gap-4">
      <Field label="Runs on" htmlFor="transcription-backend">
        <Select
          ...
          value={backend}
          onChange={(value) => onCommit({ transcriptionBackend: value === 'nemotron' ? 'nemotron' : 'openrouter' })}
          options={[
            { value: 'openrouter', label: 'OpenRouter', detail: 'Hosted transcription, billed per minute' },
            { value: 'nemotron', label: 'This computer', detail: 'Nemotron on your GPU, no API cost' }
          ]}
        />
      </Field>
